...
import pygame as pg
import threading
from bayes import BayesianNetwork, compute_conditional
...
class VictoriaBloodlines:
        ...
        while going:
            ...
            for widget in self.family_widgets:
                ...
                if widget.hover and f'G_{widget.get_name()}' in self.marginals:
                    prob_mass = self.marginals[f'G_{widget.get_name()}']                    
    ...
    def run_inference(self, evidence, name="queen_victoria"):
        self.running_bp = True
        prob_mass = compute_conditional(self.bnet, {f'G_{name}': "xX"}, evidence)
        prob_mass += compute_conditional(self.bnet, {f'G_{name}': "XX"}, evidence)   
        ...
        self.marginals = {f'G_{name}': prob_mass}
